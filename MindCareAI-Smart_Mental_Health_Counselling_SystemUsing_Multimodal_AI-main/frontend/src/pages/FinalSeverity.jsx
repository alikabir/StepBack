        ...
        return (
            <AnimatePresence mode="wait">
                <motion.div
                    ...
                    style={{ background: '#071014' }} // Dark premium bg
                >
                    <motion.div
                ...
                >
                    ...
                    >
                        <div className="relative">
                            <div className="w-16 h-16 border-4 border-[#7dd3fc]/20 border-t-[#7dd3fc] rounded-full animate-spin" style={{ boxShadow: '0 0 30px rgba(125,211,252,0.2)' }} />
                        </div>
                ...
                <div className="w-full max-w-3xl mx-auto space-y-6">
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: "easeOut" }}>
                        ...
                        <div className="text-center mb-10">
                            <h1 className="font-display tracking-tight text-4xl md:text-5xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-sky-400 to-cyan-400 text-center mb-2">
                                Final Assessment
                            </h1>
                        ...
                        <div className="space-y-6">
                            ...
                            {data.user_profile && (
                                <motion.div 
                                    ...
                                    className="bg-white/[0.03] border border-white/10 backdrop-blur-xl rounded-2xl shadow-[0_8px_32px_rgba(0,0,0,0.4)] relative overflow-hidden flex flex-col md:flex-row items-start md:items-center justify-between gap-4 p-6"
                                >
                                    ...
                                    <div className="md:text-right">
                                        <span className="px-3 py-1 rounded-full bg-sky-500/10 text-sky-400 text-[10px] uppercase tracking-[0.2em] font-bold border border-sky-500/20">{data.user_profile.occupation}</span>
                                    </div>
                                </motion.div>
                            )}
                            ...
                            <motion.div 
                                ...
                                className="bg-white/[0.03] border border-white/10 backdrop-blur-xl rounded-2xl shadow-[0_8px_32px_rgba(0,0,0,0.4)] relative overflow-hidden p-8 md:p-12 text-center"
                            >
                            ...
                            <motion.div 
                                ...
                                className="bg-white/[0.03] border border-white/10 backdrop-blur-xl rounded-2xl shadow-[0_8px_32px_rgba(0,0,0,0.4)] relative overflow-hidden p-8 group hover:bg-white/[0.04] transition-colors"
                            >
                            ...
                            >
                                {[
                                    ['ðŸ§  Behaviour Model', data.behaviour_score, 'bg-cyan-500', 'shadow-[0_0_15px_#06b6d4]', 'behaviour', data.detailed_records?.behaviour],
                                    ['ðŸ’¬ Conversational AI', data.chat_score, 'bg-sky-500', 'shadow-[0_0_15px_#10b981]', 'chat', data.detailed_records?.chat],
                                    ['ðŸ˜ Facial Emotion', data.face_score, 'bg-purple-500', 'shadow-[0_0_15px_#a855f7]', 'face', data.detailed_records?.face],
                                    ['ðŸŽ™ï¸ Voice Analysis', data.voice_score, 'bg-amber-500', 'shadow-[0_0_15px_#f59e0b]', 'voice', data.detailed_records?.voice]
                                ].map(([label, score, bgClass, shadowClass, key, details]) => {
                                    const isExpanded = expandedCard === key;
                                    return (
                                        <div key={label} className={`bg-white/[0.03] border backdrop-blur-xl rounded-2xl shadow-[0_8px_32px_rgba(0,0,0,0.4)] relative overflow-hidden transition-all duration-300 hover:bg-white/[0.04] hover:border-white/10 ${isExpanded ? 'border-white/20' : 'border-white/10'}`}>
                                            {/* Header */}
                                            <div
                            ...
                            {data.risk_level === 'Low' && (
                                <motion.div className="bg-sky-950/30 border border-sky-500/30 backdrop-blur-xl rounded-2xl p-6 text-center shadow-[0_0_30px_rgba(14,165,233,0.1)]"
                                    initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}>
                                    <div className="w-16 h-16 mx-auto bg-sky-500/20 rounded-full flex items-center justify-center mb-3">
                                        <CheckCircle size={32} className="text-sky-400" />
                                    </div>
                                    <h3 className="text-sky-400 font-bold text-lg mb-1 tracking-wide">Rest & Recovery Optimized</h3>
                                    <p className="text-sky-200/70 text-sm">Baseline telemetry indicates low stress. Continue maintaining your current behavioral patterns.</p>
                                </motion.div>
                            )}
                                ...
                                <motion.button onClick={() => nav('/dashboard')}
                                    ...
                                    className={`w-full py-5 rounded-2xl text-lg flex items-center justify-center gap-3 transition-all duration-300
                                                ${emergency && !acknowledged 
                                                    ...
                                                    : 'bg-gradient-to-r from-[#7dd3fc] to-[#38bdf8] text-black font-bold uppercase tracking-widest hover:shadow-[0_0_30px_rgba(125,211,252,0.4)] shadow-[0_0_20px_rgba(125,211,252,0.2)]'
                                                }`}
