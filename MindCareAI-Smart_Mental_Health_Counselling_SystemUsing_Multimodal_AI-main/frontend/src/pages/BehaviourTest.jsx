...
const THEMES = {
    Low: { color: '#10B981', bg: 'bg-sky-500/10', border: 'border-sky-500/30', text: 'text-sky-400', glow: 'glow-green', icon: CheckCircle, message: 'Excellent wellness profile! Your habits are contributing to a healthy lifestyle.' },
...
function Slider({ label, name, value, min, max, onChange, colorMap }) {
    ...
    const color = colorMap ? colorMap(pct, value) : '#38bdf8' // dynamic track color
    return (
        <div className="group">
            ...
            <div className="relative w-full h-4 flex items-center">
                ...
                <div className="absolute w-5 h-5 rounded-full border-2 pointer-events-none group-hover:scale-110 shadow-lg transition-transform"
                    style={{ left: `calc(${pct}% - 10px)`, borderColor: color, backgroundColor: '#0b1114', boxShadow: `0 0 15px ${color}60`, zIndex: 5 }} />
...
export default function BehaviourTest() {
    ...
    return (
        <>
            <AnimatePresence>
                {showPreloader && (
                    <motion.div
                        ...
                        className="fixed inset-0 z-[9999] bg-[#0b1114] flex items-center justify-center pointer-events-none"
                    >
                        <motion.div
                        ...
                        >
                            <div className="relative flex items-center justify-center w-20 h-20 rounded-2xl bg-sky-900/20 border border-sky-500/30 overflow-hidden shadow-[0_0_40px_rgba(125,211,252,0.2)]">
                                <motion.div
                                    animate={{ top: ['100%', '-10%'] }}
                                    transition={{ duration: 1.5, repeat: Infinity, ease: 'linear' }}
                                    className="absolute left-0 right-0 h-1 bg-sky-400 shadow-[0_0_20px_#38bdf8]"
                                />
                                <Brain size={32} className="text-sky-400 z-10" />
                            </div>
                            <h2 className="text-white font-display text-2xl md:text-3xl font-bold tracking-[0.2em] relative">
                                STEP <span className="text-sky-400">1</span> : BEHAVIOUR TEST
                            </h2>
            ...
            <div className="relative flex flex-col items-center justify-center min-h-screen py-16 px-4 bg-[#0b1114] overflow-hidden font-sans">
                ...
                <div className="w-full max-w-[1000px] mx-auto z-10 relative">
                    ...
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mt-8">
                        <div className="text-center mb-10 print:hidden">
                            <h1 className="text-4xl md:text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-sky-400 to-cyan-500 mb-3 tracking-tight font-display">Behaviour Analysis</h1>
                            <p className="text-slate-400 text-lg font-medium opacity-80">Step 1/4 â€” AI-powered mental health behaviour prediction</p>
                        </div>

                        {!result ? (
                            <div className="bg-[#0a0f12]/80 backdrop-blur-2xl border border-white/[0.04] shadow-[0_0_50px_rgba(0,0,0,0.6)] rounded-2xl p-10 md:p-12 space-y-8 relative overflow-hidden mx-auto w-full at-noise">
                                {/* Inner glow */}
                                <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[80%] h-[500px] bg-sky-500/5 blur-[120px] rounded-[100%] pointer-events-none" />
                                ...
                                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 pb-8 border-b border-white/[0.05] relative z-10">
                                    <div>
                                        <h2 className="text-xl md:text-2xl font-bold text-white flex items-center gap-3 font-display">
                                            <div className="p-2 bg-sky-500/10 rounded-xl border border-sky-500/20 shadow-[0_0_15px_rgba(14,165,233,0.1)]">
                                    ...
                                    {/* <button
                                        ...
                                        className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-bold transition-all border ${manualMode ? 'bg-orange-500/20 border-orange-500 text-orange-400 shadow-[0_0_15px_rgba(249,115,22,0.3)]' : 'bg-white/5 border-white/10 text-slate-400 hover:text-sky-400 hover:border-sky-500/30'}`}
                                    >
                                ...
                                <div className="space-y-10">
                                    {/* Identity Section */}
                                    <div className={`grid grid-cols-1 md:grid-cols-4 gap-6 p-8 rounded-2xl border transition-all ${manualMode ? 'bg-black/20 border-sky-500/20 shadow-[0_0_20px_rgba(14,165,233,0.05)]' : 'bg-black/40 border-white/5 opacity-50 pointer-events-none'}`}>
                                        ...
                                        <div className="field-group">
                                            ...
                                            {manualMode ? (
                                                <select className="bg-transparent border-none text-white font-bold w-full focus:ring-0 p-0 appearance-none text-xl tracking-wide cursor-pointer"
                                                    ...
                                                    {GENDER_OPTIONS.map(o => <option key={o} value={o} className="bg-[#0b1114] text-white">{o}</option>)}
                                                </select>
                                            ) : (
                                        ...
                                        <div className="field-group">
                                            ...
                                            {manualMode ? (
                                                <select className="bg-transparent border-none text-white font-bold w-full focus:ring-0 p-0 appearance-none text-xl tracking-wide cursor-pointer"
                                                    ...
                                                    {OCCUPATION_OPTIONS.map(o => <option key={o} value={o} className="bg-[#0b1114] text-white">{o}</option>)}
                                                </select>
                                            ) : (
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                            ...
                                            <div className="flex gap-3">
                                                {BMI_OPTIONS.map(o => (
                                                    <button key={o} onClick={() => update('bmi_category', o)}
                                                        className={`flex-1 py-3.5 text-[10px] font-black rounded-xl border transition-all ${form.bmi_category === o ? 'bg-sky-500/20 text-sky-400 border-sky-500 shadow-[0_0_15px_rgba(14,165,233,0.3)] ring-1 ring-sky-400/50' : 'bg-white/5 border-white/10 text-slate-400 hover:text-sky-400 hover:border-sky-500/30'}`}>
                                                        {o}
                                                    </button>
                                                ))}
                                            </div>
                                        ...
                                        <div className="field-group">
                                            <label className="text-slate-300 text-sm font-bold block mb-4 flex items-center gap-3">
                                                <div className="w-2 h-2 rounded-full bg-cyan-400 shadow-[0_0_8px_rgba(45,212,191,0.5)]" />
                                                Physical Activity Level (1-100)
                                            </label>
                                            <div className="relative">
                                                <input type="number"
                                                    className={`w-full bg-black/20 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-slate-500 focus:outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500 transition-all font-sans pr-12 ${(form.physical_activity < 1 || form.physical_activity > 100) ? 'border-red-500/50 focus:ring-red-500/30' : ''}`}
                                            ...
                                            {(form.physical_activity < 1 || form.physical_activity > 100) && (
                                                <p className="text-red-400 text-[9px] uppercase font-bold mt-2 tracking-[0.2em]">âš ï¸ Must be between 1 and 100</p>
                                            )}
                                        </div>
                                    </div>

                                    {/* Lifestyle Sliders */}
                                    <div className="grid grid-cols-1 gap-12 bg-black/20 p-8 rounded-2xl border border-white/[0.05] shadow-inner relative z-10">
                                    ...
                                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
                                        {[
                                            ...
                                            <div key={key} className="bg-black/20 p-8 rounded-2xl border border-white/5 text-center transition-all hover:border-sky-500/30 hover:shadow-[0_0_20px_rgba(14,165,233,0.1)] group">
                                                <label className="text-[11px] font-black text-slate-400 uppercase tracking-[0.1em] block mb-2">{label}</label>
                                                <div className="flex items-baseline justify-center gap-1">
                                                    <input type="number" className="bg-transparent border-none text-white font-black text-4xl w-28 text-center focus:outline-none focus:ring-0 p-0 group-hover:text-sky-400 transition-colors"
                                    ...
                                    <div className="pt-6">
                                        <motion.button onClick={handleSubmit} disabled={loading}
                                            className="w-full py-5 flex items-center justify-center gap-3 bg-gradient-to-r from-sky-500 to-cyan-400 text-white font-extrabold rounded-xl transition-all shadow-lg hover:shadow-[0_0_20px_rgba(14,165,233,0.4)] disabled:opacity-50 disabled:cursor-not-allowed uppercase tracking-wide font-display text-sm"
                                            ...
                                            ) : <>ðŸ§  Run AI Behaviour Intelligence Analysis <Zap size={18} fill="currentColor" /></>}
                                        </motion.button>
                            ...
                            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="space-y-6 print:block print:bg-white print:shadow-none print:border-none print:m-0 print:p-0" style={{ WebkitPrintColorAdjust: 'exact', printColorAdjust: 'exact' }}>
                                {(() => {
                                    ...
                                    return (
                                        <div className={`bg-white/[0.03] backdrop-blur-2xl border border-white/10 shadow-[0_0_40px_rgba(0,0,0,0.5)] rounded-2xl p-8 ${theme.border} ${theme.glow} print:border print:border-gray-300 print:bg-transparent print:shadow-none print:break-inside-avoid print:p-6`}>
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                <div className="bg-black/20 p-5 rounded-2xl border border-white/5 space-y-4 print:border print:border-gray-300 print:bg-transparent print:shadow-none print:break-inside-avoid">
                                                    <div className="flex justify-between items-center text-xs font-bold text-slate-400 uppercase tracking-widest print:text-gray-700">
                                                        ...
                                                        <span className="text-sky-400 font-black">{result?.confidence?.toFixed(1)}%</span>
                                                    </div>
                                                    <div className="h-2 bg-white/10 rounded-full overflow-hidden print:bg-gray-200">
                                                        <motion.div className="h-full bg-gradient-to-r from-sky-400 to-cyan-500"
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                    <div className="lg:col-span-2 bg-white/[0.03] backdrop-blur-2xl rounded-2xl p-6 border border-white/10 shadow-[0_0_30px_rgba(0,0,0,0.3)] print:border print:border-gray-300 print:bg-transparent print:shadow-none print:break-inside-avoid">
                                        <h3 className="font-bold text-slate-200 mb-6 flex items-center gap-2 text-sm uppercase tracking-widest opacity-80 print:text-black"><BarChart2 className="text-sky-400" size={16} /> Metric Visualization</h3>
                                        <div className="h-[240px] w-full">
                                            <ResponsiveContainer width="100%" height="100%">
                                                <BarChart data={[
                                                    ...
                                                    { name: 'Activity', val: form.physical_activity / 10, max: 10, color: '#fb7185' },
                                                    ...
                                                    <Tooltip contentStyle={{ background: '#0b1114', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '12px' }} />
                                    ...
                                    <div className="bg-white/[0.03] backdrop-blur-2xl rounded-2xl p-6 border border-white/10 shadow-[0_0_30px_rgba(0,0,0,0.3)] space-y-4 print:border print:border-gray-300 print:bg-transparent print:shadow-none print:break-inside-avoid">
                                        <h3 className="font-bold text-slate-200 mb-4 flex items-center gap-2 text-sm uppercase tracking-widest opacity-80 print:text-black"><Zap className="text-sky-400" size={16} /> Key Factors</h3>
                                        {[
                                            ...
                                            return (
                                                <div key={f.label} className={`p-4 rounded-xl border ${isBad ? 'border-red-500/20 bg-red-500/5' : 'border-white/5 bg-black/20'} print:border-gray-200 print:bg-transparent`}>
                                                    <div className="flex justify-between items-center mb-1 text-[10px] uppercase font-bold tracking-widest">
                                                        ...
                                                        <span className={isBad ? 'text-red-500 print:text-red-500' : 'text-sky-500 print:text-sky-500'}>{isBad ? 'Alert' : 'Good'}</span>
                                                    </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div className="bg-white/[0.03] backdrop-blur-2xl rounded-2xl p-6 border border-white/10 flex flex-col justify-between shadow-[0_0_30px_rgba(0,0,0,0.3)] print:border print:border-gray-300 print:bg-transparent print:shadow-none print:break-inside-avoid">
                                        <div>
                                            <h3 className="font-bold text-slate-200 mb-4 text-xs uppercase tracking-widest opacity-80 italic print:text-black">ðŸ” Prediction Insight</h3>
                                            <p className="text-slate-300 text-sm leading-relaxed print:text-gray-800">
                                                Your pattern indicates a <span className="text-sky-400 font-bold">{result?.behaviour_risk?.toLowerCase()}</span> risk.
                                        ...
                                        <button onClick={() => window.print()} className="w-full py-3 mt-6 rounded-xl bg-white/5 text-[10px] font-bold text-slate-400 uppercase border border-white/10 hover:text-white hover:bg-white/10 hover:border-sky-500/30 transition-all print:hidden">Export Analysis Report</button>
                                    </div>
                                    <div className="bg-white/[0.03] backdrop-blur-2xl rounded-2xl p-6 border border-white/10 shadow-[0_0_30px_rgba(0,0,0,0.3)] print:border print:border-gray-300 print:bg-transparent print:shadow-none print:break-inside-avoid">
                                        <h3 className="font-bold text-slate-200 mb-4 text-xs uppercase tracking-widest opacity-80 italic print:text-black">ðŸ’¡ AI Recommendations</h3>
                                        <div className="space-y-3">
                                            {result?.recommendations?.slice(0, 3).map((r, i) => (
                                                <div key={i} className="flex gap-3 text-xs text-slate-300 leading-normal p-3 rounded-xl bg-black/20 border border-white/5 print:border-gray-200 print:bg-transparent print:text-gray-800">
                                                    <span className="text-sky-400 text-sm mt-[1px]">âœ¦</span> {r}
                                                </div>
                                            ))}
                                        </div>
                                    </div>
