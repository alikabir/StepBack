...
export default function ChatCounselling() {
    ...
    useEffect(() => {
        api.get('/api/chat/history').then(r => {
            if (r.data.length > 0) {
                ...
                setMessages([{
                    id: 1, sender: 'bot',
                    message: `Hi ${user?.full_name || 'there'}! ðŸ‘‹ I'm your MindBridge AI counsellor.\n\nI'm here to listen and help you. **What is your problem?**`,
            ...
            setMessages([{
                id: 1, sender: 'bot',
                message: `Hi ${user?.full_name}! ðŸ‘‹ I'm your MindBridge AI counsellor. I'm here to listen and help you. What is your problem?`,
                timestamp: new Date().toISOString()
            }])
        })
    ...
    const send = async () => {
        ...
        try {
            ...
            setMessages(p => {
                ...
                return [...filtered, { id: Date.now() + 2, sender: 'bot', message: "âš ï¸ Request failed. Please try again.", timestamp: new Date().toISOString() }];
            });
            setInput(sentInput);
        } finally {
            setLoading(false);
    ...
    const resetChat = async () => {
        try {
            ...
            setMessages([{
                id: Date.now(),
                sender: 'bot',
                message: `Hi ${user?.full_name || 'there'}! ðŸ‘‹ I'm resetting our session to give us a fresh start. What would you like to talk about?`,
                timestamp: new Date().toISOString()
            }])
            setEmergency(false)
    ...
    const getEmotionEmoji = (text) => {
        ...
        if (lower.includes('sad') || lower.includes('unhappy') || lower.includes('lonely') || lower.includes('depressed')) return 'ðŸ˜¢';
        if (lower.includes('happy') || lower.includes('glad') || lower.includes('great') || lower.includes('good')) return 'ðŸ˜Š';
        if (lower.includes('angry') || lower.includes('mad') || lower.includes('frustrat')) return 'ðŸ˜ ';
        if (lower.includes('anxious') || lower.includes('worry') || lower.includes('scared') || lower.includes('fear')) return 'ðŸ˜°';
        return null;
    }
    ...
    const SummaryCard = ({ data }) => (
        <motion.div
            ...
            className="w-full max-w-[950px] mx-auto mt-6 bg-[#0a0f12]/80 backdrop-blur-2xl p-8 space-y-6 relative overflow-hidden group shadow-[0_0_50px_rgba(0,0,0,0.6)] border border-white/[0.04] rounded-2xl"
        >
            <div className="absolute top-0 right-0 w-32 h-32 bg-sky-500/10 blur-[60px] rounded-full -mr-16 -mt-16 group-hover:bg-sky-500/20 transition-colors duration-700 pointer-events-none" />

            <div className="flex items-center gap-4 mb-2 relative z-10">
                <div className="p-3 bg-sky-500/10 rounded-xl border border-sky-500/20 shadow-[0_0_15px_rgba(14,165,233,0.1)]">
                ...
                <div>
                    ...
                    <p className="text-[10px] text-sky-400/80 font-black uppercase tracking-[0.2em] mt-1">Based on your conversation</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 relative z-10">
                {[
                    ...
                    { label: 'Risk Level', value: data.risk_level || 'Low', color: data.risk_level === 'High' ? 'text-red-400' : data.risk_level === 'Medium' ? 'text-yellow-400' : 'text-sky-400' },
                    ...
                    <div key={i} className="flex items-center justify-between p-4 bg-black/20 border border-white/5 rounded-2xl hover:bg-white/5 transition-colors group/item">
                        ...
                        <span className={`text-sm font-bold ${item.color || 'text-white'} group-hover/item:text-sky-400 transition-colors`}>
                            {item.value || 'N/A'}
                        </span>
                    </div>
                ))}
                ...
                {data.additional_notes && data.additional_notes.trim() !== '' && (
                    <div className="col-span-1 md:col-span-2 flex flex-col items-start p-4 bg-black/20 border border-white/5 rounded-2xl hover:bg-white/5 transition-colors group/item mt-2">
                        ...
                        <span className="text-sm font-medium text-slate-200 group-hover/item:text-sky-400 transition-colors leading-relaxed">
            ...
            <div className="pt-4 relative z-10">
                ...
                <button
                    onClick={() => nav('/face')}
                    className="w-full py-5 text-sm font-bold uppercase tracking-widest bg-sky-500 text-[#000000] rounded-xl shadow-[0_0_16px_rgba(125,211,252,0.35)] flex items-center justify-center gap-4 group hover:shadow-[0_0_24px_rgba(125,211,252,0.5)] transition-all"
                >
                    <span className="relative z-10 flex items-center gap-3">
                        Proceed to Next Step â†’ Face Analysis <CheckCircle size={18} fill="currentColor" />
                    </span>
                </button>
            </div>
        </motion.div>
    ...
    return (
        <>
            ...
            <div className="relative flex flex-col items-center justify-center py-16 px-4 overflow-hidden" style={{ minHeight: '100vh', fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif", background: '#071014' }}>
                {/* The Root Background */}
                ...
                >
                    <div className="flex flex-col gap-6 w-full">
                        ...
                        >
                            ...
                            Back to Step 1 â€“ Behaviour Analysis
                        </button>
                    ...
                    <div className="text-center space-y-3">
                        ...
                        <p className="text-lg font-medium opacity-80" style={{ color: 'rgba(148,163,184,0.72)' }}>Step 2/4 â€” Share your thoughts in a safe and private space</p>
                    ...
                    <div className="w-full max-w-4xl mx-auto h-[80vh] flex flex-col overflow-hidden"
                        ...
                        <div className="p-6 flex items-center justify-between z-10 relative" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                            <div className="flex flex-col">
                                <h3 className="leading-tight flex items-center gap-2 text-lg" style={{ fontFamily: "'Space Grotesk', sans-serif", fontWeight: 700, color: '#f0f0f0' }}>
                                    AI Counsellor
                                    <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#7dd3fc', boxShadow: '0 0 10px rgba(125,211,252,0.8)' }} className="animate-pulse" />
                                </h3>
                            </div>
                        ...
                        >
                            ...
                            {messages.map((msg) => (
                                ...
                                >
                                    {msg.sender === 'bot' && (
                                        <div className="flex-shrink-0 w-8 h-8 rounded-full bg-sky-500/20 border border-sky-500/40 flex items-center justify-center mr-3 mt-1">
                                    ...
                                    <div className={`flex flex-col w-fit max-w-[85%] md:max-w-[70%] ${msg.sender === 'user' ? 'items-end' : 'items-start'}`}>
                                        <div
                                            ...
                                            style={msg.sender === 'user'
                                                ? { background: 'linear-gradient(135deg, #7dd3fc, #38bdf8)', color: '#000000', borderRadius: '16px 0 16px 16px', padding: '16px 20px', fontSize: '15px', fontWeight: 500, boxShadow: '0 4px 20px rgba(125,211,252,0.15)' }
                                        ...
                                        >
                                            {msg.sender === 'bot' && !loading && msg.message && (
                                                <button
                                                    ...
                                                    style={{ background: 'rgba(255,255,255,0.1)', color: copiedId === msg.id ? '#7dd3fc' : '#cbd5e1' }}
                                                    title="Copy Message"
                                                >
                                        ...
                                        <div className={`mt-2 flex items-center gap-2 text-[10px] tabular-nums ${msg.sender === 'user' ? 'justify-end pr-1' : 'justify-start pl-1'}`} style={{ opacity: 0.4, fontWeight: 400 }}>
                                            ...
                                            {msg.sender === 'user' && <CheckCircle size={10} style={{ color: '#7dd3fc' }} />}
                                        </div>
                                    </div>
                        ...
                        <div className="px-6 py-5 relative z-10 bg-transparent flex items-end gap-4 w-full" style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }}>
                            <textarea
                                ...
                                style={{
                                    ...
                                    border: isFocused ? '1px solid #7dd3fc' : '1px solid rgba(255,255,255,0.1)',
                                    color: '#fff',
                                    fontSize: '14px',
                                    lineHeight: '1.5',
                                    minHeight: '52px',
                                    boxShadow: isFocused ? '0 0 0 1px #7dd3fc' : 'none',
                                    overflowY: 'auto'
                                }}
                            ...
                            {loading && !showSummary ? (
                                ...
                                <motion.button
                                    ...
                                    style={(!input.trim() || showSummary)
                                        ...
                                        : { background: 'rgba(125,211,252,0.15)', border: '1px solid rgba(125,211,252,0.4)', color: '#7dd3fc', boxShadow: '0 0 15px rgba(125,211,252,0.3)', height: '52px' }
                                    }
                                    onMouseEnter={e => {
                        ...
                        {(messages.length >= 8 || chatComplete) && !showSummary && (
                            ...
                            >
                                <button
                                    ...
                                    onMouseEnter={e => e.currentTarget.style.boxShadow = '0 0 30px rgba(125,211,252,0.5)'}
                                    onMouseLeave={e => e.currentTarget.style.boxShadow = '0 8px 25px rgba(125,211,252,0.3)'}
                                >
                                    <span className="relative z-10 flex items-center gap-3">
