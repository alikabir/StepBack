...
def get_bmi_hint(bmi: str) -> str:
    hints = {
        "Underweight": "âš ï¸ Being underweight can affect energy and mental health. Consider nutritional support.",
        "Normal": "âœ… Healthy BMI range. Keep maintaining balanced nutrition.",
        "Healthy Weight": "âœ… Healthy BMI range. Keep maintaining balanced nutrition.",
        "Overweight": "âš ï¸ Being overweight can increase stress. Light exercise recommended.",
        "Obese": "âš ï¸ Obesity significantly impacts mental and physical health. Medical guidance recommended."
    }
    return hints.get(bmi, "")

def get_sleep_warning(hours: float, quality: int) -> str:
    if hours < 6:
        return "âš ï¸ Sleep deficiency detected. Less than 6 hours is insufficient for mental recovery."
    elif quality < 5:
        return "âš ï¸ Poor sleep quality detected. Consider improving sleep hygiene."
    return ""

def get_hr_warning(hr: int) -> str:
    if hr < 60:
        return "âš ï¸ Bradycardia detected. Heart rate below 60 BPM may indicate stress or cardiac issues."
    elif hr > 100:
        return "âš ï¸ Tachycardia detected. Elevated heart rate may indicate stress or anxiety."
    return ""
