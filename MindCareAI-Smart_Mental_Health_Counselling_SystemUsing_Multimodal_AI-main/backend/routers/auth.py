...
def forgot_password(req: ForgotPasswordRequest, db: Session = Depends(get_db)):
    user = db.query(User).filter(User.email == req.email).first()
    if not user:
        # Explicit debug error â€” bypasses generic security message for now
        return {"error": "User email not found in database"}

    otp = str(random.randint(100000, 999999))
    expires = datetime.now(timezone.utc) + timedelta(minutes=10)

    # â”€â”€ Log OTP immediately â€” visible in Uvicorn/HF Space logs at WARNING level â”€â”€
    logger.warning(f"ðŸš¨ DEMO MODE: OTP for requested email is {otp} ðŸš¨")
    ...
    # Commit OTP to DB first â€” guaranteed even if email sending fails
    db.commit()

    # Attempt email â€” failures are logged silently, never returned as 500
    try:
        send_otp_email(req.email, otp)
    except Exception as e:
        logger.error(f"âŒ Email send exception in route (non-fatal): {e}")
