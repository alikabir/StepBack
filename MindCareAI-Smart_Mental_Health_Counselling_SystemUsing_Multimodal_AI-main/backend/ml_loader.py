...
def load_behaviour_models():
    ...
    try:
        ...
        if not os.path.exists(model_path):
            logger.error(f"âŒ Behaviour model file not found: {model_path}")
            return False
        if not os.path.exists(encoder_path):
            logger.error(f"âŒ Behaviour encoders file not found: {encoder_path}")
            return False
        ...
        logger.info("âœ… Behaviour models loaded successfully")
        return True
    except Exception as e:
        logger.exception(f"âš ï¸ Behaviour models failed to load: {e}")
        return False
...
def load_face_model():
    ...
    try:
        ...
        if not os.path.exists(model_path):
            logger.error(f"âŒ Face model file not found: {model_path}")
            return False
        ...
        logger.info("âœ… Face emotion model loaded successfully")
        return True
    except Exception as e:
        logger.exception(f"âš ï¸ Face model failed to load: {e}")
        return False
    ...
    try:
        ...
        for p in paths:
            if not os.path.exists(p):
                logger.error(f"âŒ Voice component not found: {p}")
                return False
        ...
        logger.exception(f"âš ï¸ Voice models failed to load: {e}")
        return False
...
def load_all_models():
    ...
    if not load_behaviour_models():
        logger.critical("ðŸš¨ CRITICAL: Behaviour models failed to load at startup!")
...
def predict_behaviour(input_data: dict):
    ...
    if behaviour_model is None or behaviour_encoders is None:
        logger.error("âŒ Behaviour model or encoders not loaded!")
