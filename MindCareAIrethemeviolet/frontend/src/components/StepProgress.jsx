        ...
        <div className="bg-white/[0.03] backdrop-blur-2xl border border-white/10 rounded-2xl px-6 py-5 mb-10 w-full at-noise shadow-[0_0_30px_rgba(0,0,0,0.5)]">
            <div className="flex items-center justify-between relative max-w-[800px] mx-auto w-full">
                ...
                <div className="absolute left-4 right-4 top-4 -translate-y-1/2 h-[2px] z-0">
                    ...
                    <div
                        className="absolute left-0 top-0 bottom-0 bg-gradient-to-r from-purple-600 to-purple-400 transition-all duration-700 ease-in-out shadow-[0_0_15px_rgba(16,185,129,0.5)] rounded-full"
                    ...
                    <div key={s} className="relative z-10 flex flex-col items-center gap-3">
                        <div
                            className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold border-2 transition-all duration-500 ${i < current
                                ? 'bg-[#0c0a16] border-purple-500 text-purple-400 shadow-[0_0_15px_rgba(16,185,129,0.3)]'
                                : i === current
                                    ? 'bg-purple-500/20 border-purple-400 text-purple-400 shadow-[0_0_20px_rgba(16,185,129,0.5)]'
                                    : 'bg-[#0c0a16] border-slate-800 text-slate-600'
                                }`}
                        >
                            {i < current ? '✓' : i + 1}
                        </div>
                        <span
                            className={`text-[9px] font-black uppercase tracking-widest hidden sm:block ${i <= current ? 'text-purple-400/90' : 'text-slate-600'
                                }`}
                        >
                            {s}
                        </span>
