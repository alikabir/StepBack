...
const getScoreColorClass = (score) => {
    ...
    if (score <= 3) return 'text-purple-400';
...
const CustomTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
        return (
            <div className="bg-slate-900 border border-purple-500/30 p-3 rounded-xl shadow-xl">
...
export default function Dashboard() {
    ...
    if (loading) {
        return (
            <div className="fixed inset-0 z-[9999] bg-[#0b0914] flex flex-col items-center justify-center">
                <div className="relative flex items-center justify-center w-20 h-20 rounded-2xl bg-purple-900/20 border border-purple-500/30 mb-6 shadow-[0_0_40px_rgba(168, 85, 247,0.2)]">
                    <Loader2 size={32} className="text-purple-400 animate-spin z-10" />
                </div>
                <h2 className="text-white font-display text-2xl font-bold tracking-[0.2em]">
                    LOADING <span className="text-purple-400">DATA</span>
                </h2>
            </div>
        )
    }
    ...
    return (
        <>
            <motion.div
                ...
                className={`fixed inset-0 z-[9999] bg-[#0b0914] flex items-center justify-center pointer-events-none ${!showPreloader ? 'pointer-events-none opacity-0' : ''}`}
            ...
            <div className="min-h-screen bg-[#0b0914] text-slate-200 font-sans relative overflow-x-hidden">
                    ...
                    <div className="flex justify-between items-center mb-10 relative">
                        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}>
                            <div className="flex items-center gap-3 mb-1">
                                <h1 className="font-display text-4xl md:text-5xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-purple-400 via-teal-300 to-fuchsia-400">
                                    Welcome back, {data.user?.full_name?.split(' ')[0] || "Friend"}
                                </h1>
                                <span className="hidden sm:inline-flex items-center gap-1.5 px-3 py-1 bg-purple-500/10 border border-purple-500/30 rounded-full text-[10px] font-bold text-purple-400 uppercase tracking-widest">
                        ...
                        <div className="flex items-center gap-4">
                            <button onClick={() => window.location.href = '/behaviour'} className="hidden sm:flex items-center gap-2 bg-gradient-to-br from-[#a855f7] to-[#7c3aed] text-black shadow-[0_0_20px_rgba(168, 85, 247,0.3)] hover:shadow-[0_0_30px_rgba(168, 85, 247,0.5)] px-5 py-2.5 rounded-full font-bold transition-all">
                                <RefreshCw size={18} /> New Assessment
                            </button>

                            <div className="relative">
                                <button onClick={() => setShowProfileMenu(!showProfileMenu)} className="w-12 h-12 rounded-full bg-slate-800 border-2 border-purple-500/50 flex items-center justify-center hover:bg-slate-700 transition-colors">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:items-start relative">
                        ...
                        <div className="lg:col-span-2 space-y-8">
                            ...
                            <div className="flex items-center gap-3">
                                <span className="text-[10px] font-bold text-purple-400/70 uppercase tracking-[0.3em]">Assessment Summary</span>
                            ...
                            {data.historical_trends && data.historical_trends.length > 0 && (
                                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="bg-white/[0.03] border border-white/[0.07] backdrop-blur-xl transform-gpu will-change-transform rounded-3xl shadow-[0_8px_32px_rgba(0,0,0,0.4)] overflow-hidden p-6 md:p-8 relative transition-all duration-500 hover:border-purple-500/30 hover:shadow-[0_0_50px_rgba(167, 139, 250,0.12)] group">
                                    <div className="flex items-center justify-between mb-6">
                                        <h3 className="text-lg font-bold flex items-center gap-2.5"><Activity className="text-purple-400" size={18} /> Progress Over Time</h3>
                                    ...
                                    {data.historical_trends.length === 1 && (
                                        <div className="absolute inset-0 flex items-center justify-center z-10 pointer-events-none mb-10">
                                            <div className="bg-black/60 backdrop-blur-md border border-purple-500/30 text-purple-200 px-6 py-3 rounded-full text-sm font-bold tracking-wide shadow-[0_0_30px_rgba(167, 139, 250,0.2)]">
                                    ...
                                    <div className="h-64 w-full">
                                        <ResponsiveContainer width="100%" height="100%">
                                            <AreaChart data={data.historical_trends} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
                                                ...
                                                <Area type="monotone" dataKey="score" name="Severity" stroke="#a78bfa" strokeWidth={4} fillOpacity={1} fill="url(#colorScore)" activeDot={{ r: 8, fill: '#0b0914', stroke: '#a78bfa', strokeWidth: 3 }} dot={{ fill: '#0b0914', stroke: '#a78bfa', strokeWidth: 2, r: 5 }} filter="url(#glow)" />
                                            </AreaChart>
                                        </ResponsiveContainer>
                                    </div>
                                </motion.div>
                            ...
                            <div className="flex items-center gap-3 mt-2">
                                <span className="text-[10px] font-bold text-purple-400/70 uppercase tracking-[0.3em]">Wellness Tools</span>
                                <div className="flex-1 h-px bg-gradient-to-r from-purple-500/20 to-transparent" />
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                {/* Smart Suggestions */}
                                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="space-y-4">
                                    <h3 className="text-lg font-bold flex items-center gap-2.5"><Sparkles className="text-purple-400" size={18} /> Smart Suggestions</h3>
                                            ...
                                            <>
                                                {suggestions.lifestyle?.map((sug) => (
                                                    <div key={sug} className="flex gap-3 text-slate-300 text-sm bg-white/[0.03] border border-white/5 hover:bg-white/[0.06] transition-colors p-4 rounded-2xl relative z-10">
                                                        <div className="w-1.5 h-1.5 rounded-full bg-purple-400 mt-1.5 flex-shrink-0 shadow-[0_0_8px_#a78bfa]" />
                                                        <span className="leading-relaxed">{sug}</span>
                                                    </div>
                                                ))}
                                                {suggestions.quotes?.[0] ? (
                                                    <div className="mt-6 pt-5 border-t border-white/[0.05] flex gap-4 text-purple-200/80 italic text-sm relative z-10">
                                                        <Quote size={20} className="flex-shrink-0 text-purple-500/50 fill-purple-500/20" />
                                    ...
                                    <div className="mt-6 space-y-3">
                                        <h3 className="text-lg font-bold flex items-center gap-2.5"><Smile className="text-purple-400" size={18} /> Quick Check-in</h3>
                                            ...
                                            <div className="flex justify-between items-center">
                                                {['😫', '😟', '😐', '🙂', '🤩'].map((emoji, index) => (
                                                    <button
                                                        ...
                                                        className="w-12 h-12 flex items-center justify-center text-2xl bg-white/5 border border-white/10 rounded-full hover:scale-110 hover:bg-purple-500/20 hover:border-purple-500/50 transition-all cursor-pointer shadow-lg"
                                                    >
                                                        {emoji}
                                                    </button>
                                                ))}
                                ...
                                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="space-y-4">
                                            ...
                                            {weekDays.map((d, idx) => (
                                                <div
                                                    ...
                                                    className={`flex flex-col items-center justify-center py-3 px-1 rounded-2xl transition-all ${d.isToday
                                                        ? 'bg-gradient-to-b from-[#a855f7] to-[#7c3aed] text-black shadow-[0_0_20px_rgba(168, 85, 247,0.3)]'
                                    ...
                                    <div className="bg-white/[0.03] border border-white/[0.07] backdrop-blur-xl transform-gpu will-change-transform rounded-3xl shadow-[0_8px_32px_rgba(0,0,0,0.4)] p-6 md:p-8">
                                                ...
                                                <div className="mb-6">
                                                    ...
                                                    <div className="h-1.5 w-full rounded-full bg-white/5 overflow-hidden">
                                                        <motion.div
                                                            ...
                                                            style={{ boxShadow: pct > 0 ? '0 0 8px rgba(167, 139, 250,0.6)' : 'none' }}
                                                        />
                                                    </div>
                                                </div>
                                            )
                                        ...
                                        <div className="relative">
                                            ...
                                            <div className="absolute left-[19px] top-5 bottom-5 w-0.5 bg-gradient-to-b from-purple-500/40 via-slate-700/60 to-transparent" />
                                                    ...
                                                    >
                                                        ...
                                                        <div className="relative flex-shrink-0 z-10 mt-3.5">
                                                            <motion.button
                                                                ...
                                                                className={`w-10 h-10 rounded-full flex items-center justify-center border-2 transition-all duration-300 ${task.completed
                                                                    ? 'border-purple-400 bg-purple-500/20 shadow-[0_0_16px_rgba(167, 139, 250,0.5)]'
                                                                    : 'border-slate-600 bg-slate-900 hover:border-purple-500/60'
                                                                    }`}
                                                            >
                                                                {task.completed
                                                        ...
                                                        <motion.div
                                                            ...
                                                            className={`flex-1 cursor-pointer p-4 rounded-2xl border backdrop-blur-xl transition-all duration-300 ${task.completed
                                                                ? 'bg-purple-500/[0.07] border-purple-500/30 shadow-[0_0_20px_rgba(167, 139, 250,0.12)]'
                                                            ...
                                                            <div className="flex items-center justify-between gap-3">
                                                                ...
                                                                {task.completed && (
                                                                    <motion.span
                                                                        ...
                                                                        animate={{ scale: 1, opacity: 1 }}
                                                                        className="text-[10px] font-bold text-purple-400 uppercase tracking-wider flex-shrink-0 bg-purple-500/10 px-2 py-0.5 rounded-full border border-purple-500/20"
                                                                    >
                                                                        Done ✓
                                                                    </motion.span>
                                                                )}
                        ...
                        <div className="sticky top-28 z-20 flex flex-col gap-5">
                            <div className="flex items-center gap-3 mb-1">
                                <span className="text-[10px] font-bold text-purple-400/70 uppercase tracking-[0.3em]">AI Support</span>
                                <div className="flex-1 h-px bg-gradient-to-r from-purple-500/20 to-transparent" />
                            </div>
                            <motion.div
                                initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4 }}
                                className="bg-white/[0.03] border border-white/[0.07] backdrop-blur-xl transform-gpu will-change-transform rounded-3xl shadow-[0_8px_32px_rgba(0,0,0,0.4)] overflow-hidden flex flex-col h-[500px] lg:h-[calc(100vh-14rem)] lg:max-h-[650px] transition-all duration-500 hover:border-purple-500/20"
                            >
                                ...
                                <div className="p-5 bg-gradient-to-r from-white/[0.03] to-transparent border-b border-white/[0.07] flex items-center gap-4 relative z-10">
                                    <div className="relative">
                                        <div className="w-12 h-12 rounded-full bg-[#120d1c] border border-purple-500/20 flex items-center justify-center text-2xl shadow-[0_0_20px_rgba(167, 139, 250,0.15)]">👨‍⚕️</div>
                                        <div className="absolute bottom-0 right-0 w-3.5 h-3.5 bg-purple-400 border-[3px] border-[#120d1c] rounded-full shadow-[0_0_10px_#a78bfa]"></div>
                                    </div>
                                    <div>
                                        <h3 className="font-bold text-slate-100 text-lg tracking-wide">Dr. MindCare</h3>
                                        <div className="flex items-center gap-1.5">
                                            <span className="w-1.5 h-1.5 rounded-full bg-purple-400 animate-pulse" />
                                            <span className="text-xs text-purple-400/80 font-medium uppercase tracking-widest">AI Therapist Online</span>
                                        </div>
                                    </div>
                                </div>

                                {/* Chat Messages Area */}
                                <div className="flex-1 overflow-y-auto p-5 space-y-6 custom-scrollbar relative z-10 scrollbar-thin scrollbar-thumb-purple-500/20 scrollbar-track-transparent hover:scrollbar-thumb-purple-500/40">
                                    {chatMessages.map((msg, i) => (
                                        <div key={i} className={`flex ${msg.role === 'assistant' ? 'justify-start' : 'justify-end'}`}>
                                            <div className={`max-w-[85%] p-4 rounded-2xl text-[15px] leading-relaxed relative ${msg.role === 'assistant'
                                                ...
                                                : 'bg-purple-500/20 text-purple-50 border border-purple-500/30 rounded-tr-sm shadow-[0_0_15px_rgba(167, 139, 250,0.1)] backdrop-blur-md'
                                                }`}>
                                                {msg.text}
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex justify-start">
                                            <div className="max-w-[85%] p-5 rounded-2xl rounded-tl-sm bg-white/5 border border-white/10 backdrop-blur-md">
                                                <div className="flex gap-2">
                                                    <div className="w-2h h-2 rounded-full bg-purple-400/50 animate-pulse" style={{ animationDelay: '0ms' }} />
                                                    <div className="w-2 h-2 rounded-full bg-purple-400/50 animate-pulse" style={{ animationDelay: '150ms' }} />
                                                    <div className="w-2 h-2 rounded-full bg-purple-400/50 animate-pulse" style={{ animationDelay: '300ms' }} />
                                                </div>
                                            </div>
                                        </div>
                                    )}
                                ...
                                <div className="p-4 bg-black/40 border-t border-white/[0.05] shadow-[inset_0_20px_20px_-20px_rgba(0,0,0,0.5)] relative z-10">
                                    <form onSubmit={handleSendMessage} className="relative flex items-center">
                                        <input
                                            ...
                                            className="w-full bg-white/[0.03] border border-white/10 text-slate-200 rounded-2xl pl-5 pr-14 py-4 focus:outline-none focus:border-purple-500/50 focus:bg-white/[0.05] transition-all"
                                        />
                                        <button
                                            type="submit"
                                            disabled={!chatInput.trim() || chatLoading}
                                            className="absolute right-2 p-3 bg-gradient-to-br from-[#a855f7] to-[#7c3aed] hover:from-[#7c3aed] hover:to-[#a855f7] text-black rounded-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-[0_0_15px_rgba(168, 85, 247,0.3)]"
                                        >
                                            <Send size={18} />
                                        </button>
                                    </form>
                            ...
                            >
                                ...
                                <button onClick={() => toast.success('Journal module opening soon...')} className="flex-1 py-3.5 bg-fuchsia-500/10 hover:bg-fuchsia-500/20 border border-fuchsia-500/25 text-fuchsia-400 rounded-2xl font-bold tracking-wide text-sm transition-all flex items-center justify-center gap-2 shadow-[0_0_20px_rgba(192, 38, 211,0.08)] hover:shadow-[0_0_30px_rgba(192, 38, 211,0.2)]">
                            ...
                            <div className="bg-white/[0.03] border border-white/[0.07] backdrop-blur-xl rounded-3xl shadow-[0_8px_32px_rgba(0,0,0,0.3)] p-6 text-center relative overflow-hidden group">
                                ...
                                <span className="text-[10px] font-bold text-fuchsia-400/60 uppercase tracking-[0.3em] mb-2 block">Mindfulness</span>
                                ...
                                <div className="relative w-24 h-24 mx-auto flex items-center justify-center">
                                    {/* Pulsing rings */}
                                    <div className="absolute inset-0 border-2 border-fuchsia-500/30 rounded-full animate-[ping_3s_cubic-bezier(0,0,0.2,1)_infinite]" />
                                    <div className="absolute inset-2 border-2 border-purple-500/30 rounded-full animate-[ping_4s_cubic-bezier(0,0,0.2,1)_infinite]" />

                                    {/* Core circle */}
                                    <div className="relative w-16 h-16 bg-gradient-to-br from-fuchsia-400 to-purple-400 rounded-full shadow-[0_0_20px_rgba(232, 121, 249,0.4)] animate-[pulse_4s_ease-in-out_infinite] flex items-center justify-center">
                            ...
                            <div className="space-y-3">
                                <div className="flex items-center gap-3">
                                    <h3 className="text-base font-bold flex items-center gap-2"><PlayCircle className="text-purple-400" size={16} /> Recommended</h3>
                                    <div className="flex-1 h-px bg-gradient-to-r from-purple-500/20 to-transparent" />
                                </div>
                                ...
                                <a
                                    ...
                                    className="bg-white/[0.02] border border-white/[0.05] backdrop-blur-xl rounded-2xl p-3 flex gap-4 items-center hover:bg-white/[0.06] hover:border-purple-500/30 transition-all cursor-pointer group"
                                >
                                    <div className="w-28 h-16 rounded-xl relative flex items-center justify-center overflow-hidden border border-white/10 group-hover:border-purple-500/50 transition-colors flex-shrink-0">
                                        <img
                                    ...
                                    <div>
                                        <h4 className="text-sm font-bold text-slate-200 group-hover:text-purple-400 transition-colors line-clamp-1">5-Minute Meditation You Can Do Anywhere</h4>
                                        <p className="text-xs text-purple-500/70 mt-1">{data.severity?.risk_level || "Moderate"} Risk Suggestion</p>
                                    </div>
                                </a>
                                ...
                                >
                                    ...
                                    <div>
                                        <h4 className="text-sm font-bold text-slate-200 group-hover:text-purple-400 transition-colors line-clamp-1">10-Minute Relief from Stress & Anxiety</h4>
                                        <p className="text-xs text-purple-500/70 mt-1">{data.severity?.risk_level || "Moderate"} Risk Suggestion</p>
                                    </div>
                                </a>
                                {/* Video 3 */}
                                ...
                                >
                                    ...
                                    <div>
                                        <h4 className="text-sm font-bold text-slate-200 group-hover:text-purple-400 transition-colors line-clamp-1">15-Minute Relaxing Yoga for Stress</h4>
                                        <p className="text-xs text-purple-500/70 mt-1">{data.severity?.risk_level || "Moderate"} Risk Suggestion</p>
                                    </div>
                                </a>
                                ...
                                >
                                    ...
                                    <div>
                                        <h4 className="text-sm font-bold text-slate-200 group-hover:text-purple-400 transition-colors line-clamp-1">Deep Sleep & Relaxation Music</h4>
                                        <p className="text-xs text-purple-500/70 mt-1">{data.severity?.risk_level || "Moderate"} Risk Suggestion</p>
                                    </div>
                                </a>
                            </div>
                ...
                {showEditModal && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-md p-4">
                        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-[#120d1c] border border-white/10 rounded-3xl p-8 w-full max-w-lg shadow-[0_40px_80px_rgba(0,0,0,0.5)] relative max-h-[90vh] overflow-y-auto custom-scrollbar">
                            ...
                            <form onSubmit={handleEditSave} className="space-y-4">
                                <div>
                                    ...
                                    <input id="edit-fullname" type="text" value={editForm.full_name} onChange={e => setEditForm({ ...editForm, full_name: e.target.value })} className="w-full bg-black/30 border border-white/10 rounded-lg px-4 py-2.5 text-white focus:outline-none focus:border-purple-500" required />
                                </div>
                                <div>
                                    <label htmlFor="edit-email" className="block text-sm font-medium text-slate-400 mb-1">Email</label>
                                    <input id="edit-email" type="email" value={editForm.email} onChange={e => setEditForm({ ...editForm, email: e.target.value })} className="w-full bg-black/30 border border-white/10 rounded-lg px-4 py-2.5 text-white focus:outline-none focus:border-purple-500" required />
                                </div>
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label htmlFor="edit-age" className="block text-sm font-medium text-slate-400 mb-1">Age</label>
                                        <input id="edit-age" type="number" value={editForm.age} onChange={e => setEditForm({ ...editForm, age: e.target.value })} className="w-full bg-black/30 border border-white/10 rounded-lg px-4 py-2.5 text-white focus:outline-none focus:border-purple-500" required />
                                    </div>
                                    <div>
                                        <label htmlFor="edit-gender" className="block text-sm font-medium text-slate-400 mb-1">Gender</label>
                                        <select id="edit-gender" value={editForm.gender} onChange={e => setEditForm({ ...editForm, gender: e.target.value })} className="w-full bg-black/30 border border-white/10 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-purple-500 appearance-none bg-slate-800" required>
                                ...
                                <div>
                                    ...
                                    <select
                                        ...
                                        className="w-full bg-black/30 border border-white/10 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-purple-500 appearance-none bg-slate-800"
                                        required
                                    >
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input id="edit-phone" type="text" value={editForm.phone} onChange={e => setEditForm({ ...editForm, phone: e.target.value })} className="w-full bg-black/30 border border-white/10 rounded-lg px-4 py-2.5 text-white focus:outline-none focus:border-purple-500" />
                                    </div>
                                    <div>
                                        <label htmlFor="edit-location" className="block text-sm font-medium text-slate-400 mb-1">Location</label>
                                        <input id="edit-location" type="text" value={editForm.location} onChange={e => setEditForm({ ...editForm, location: e.target.value })} className="w-full bg-black/30 border border-white/10 rounded-lg px-4 py-2.5 text-white focus:outline-none focus:border-purple-500" />
                                    </div>
                                </div>
                                <div className="pt-4 flex gap-3">
                                    ...
                                    <button type="submit" className="flex-1 py-2.5 rounded-lg bg-purple-500 hover:bg-purple-400 text-slate-900 transition-colors font-bold">Save Changes</button>
                                </div>
                            </form>
                        </motion.div>
                    </div>
